...
class ClariForecastStream(HttpStream, ABC):
    ...
    `class ClariForecastStream(HttpStream, ABC)` which is the current class
    `class Customers(ClariForecastStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(ClariForecastStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalClariForecastStream((ClariForecastStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class SourceClariForecast(AbstractSource):
    def check_connection(self, logger, config) -> Tuple[bool, any]:
        ...
        if incoming_type not in types_to_export:
            return False, f"Input {incoming_type} is invalid. Please input one of the following: {types_to_export}"
        return True, None
